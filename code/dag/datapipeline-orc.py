...
iam_role_name="AmazonMWAA-MyAirflowEnvironment-btcheD"

config = {
    "Name":"catalog-cars-data",
    "Role": "AmazonMWAA-MyAirflowEnvironment-btcheD",
    "DatabaseName":"curated-data",
    "Description":"Crawl cars dataset and catalog the the data",
    'Targets':{'S3Targets' : [{'Path': "s3://airflow-bucket-test-1/curated-data/" }]}
}
...
with DAG(
    ...
    purge_processed_data_s3_objects = BashOperator(
        ...
        bash_command=f'aws s3 rm s3://airflow-bucket-test-1/processed-data/ --recursive',
    )
    ...
    run_glue_job = GlueJobOperator(
        ...
        script_location="s3://airflow-bucket-test-1/scripts/etlscript.py",
        s3_bucket="airflow-bucket-test-1",
    ...
    sync_buckets = BashOperator(
        ...
        bash_command='aws s3 sync s3://airflow-bucket-test-1/landed-zone/  s3://airflow-bucket-test-1/processed-data/',
    )
    
    # Task to purge raw data files from S3 bucket
    purge_raw_data_file = S3DeleteObjectsOperator(
        task_id="purge_raw_data_file",
        bucket="airflow-bucket-test-1",
